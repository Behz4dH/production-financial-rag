...
> **2026-07-08 pivot:** the learner has an empty sibling project at `../training/backend` (mirroring `production-rag`'s `app/`, `core/`, `rag/`, `eval/` layout) and wants to *build the real thing there*, not do throwaway exercises. From Session 1 onward, "Exercise" means: implement the real module in `training/backend` for that session's concept, using `production-rag` as the reference for design rationale — not a scratch branch discarded afterward. Domain/corpus: same as production-rag (ERC 10-K PDFs + benchmark, copied into `training/backend/data/`). Guidance level: Claude explains the concept + tradeoffs, the learner decides the implementation, Claude flags divergences from `production-rag` and discusses why before moving on — not silent mirroring. Build order unchanged (config → providers/ingestion → retrieval/generation → API → eval → observability).
